...
test('in-app release notes and update checker stay current-only but functional', () => {
  ...
  assert.deepEqual(versions.slice(0, 3), ['3.2.23', '3.2.22', '3.2.21']);
  assert.match(releaseNotesBlock, /"3.2.23": \[/u);
  assert.doesNotMatch(releaseNotesBlock, /"3\.1\.32": \[/u);
  const currentNotes = releaseNotesBlock.match(/"3\.2\.23": \[([\s\S]*?)\],/u)?.[1] || '';
