...
test('in-app release notes and update checker stay current-only but functional', () => {
  ...
  assert.deepEqual(versions.slice(0, 3), ['3.2.27', '3.2.26', '3.2.25']);
  assert.match(releaseNotesBlock, /"3.2.27": \[/u);
  assert.doesNotMatch(releaseNotesBlock, /"3\.1\.32": \[/u);
  const currentNotes = releaseNotesBlock.match(/"3\.2\.27": \[([\s\S]*?)\],/u)?.[1] || '';
